...
'''
Same as https://github.com/niclabs/water-monitoring/blob/master/energy/decoder/payloadDecoder.py
but removed base64 intermediate step.
'''
...
def decode_payload(byte_array:bytearray):
    """
    Decodes a given bytearray into a set of measures with timestamp.
    Input: byte_array: bytearray with the bytes containing sensor's readings
    ...
    while(pointer < array_size):
        try:
            ...
            print("Error: wrong coding information on sensing unit. Byte position inside bytearray:", pointer)
            #traceback.print_exc() 
            return None
    return readings

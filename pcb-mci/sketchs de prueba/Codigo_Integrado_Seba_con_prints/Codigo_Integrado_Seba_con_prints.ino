// ----------------------------------
// Código funcional para pruebas, despues 
// revisar integración de códigos antiguos
//
...
float tdsValue() { float aux = adcReader(0); Serial.print("TDS: "); Serial.println(aux); return aux;}
float phValue() { float aux = adcReader(1); Serial.print("pH: "); Serial.println(aux); return aux;}
float turbValue() {float  aux = adcReader(2); Serial.print("Turb: "); Serial.println(aux); return aux; }
float presValue() { float aux = adcReader(3); Serial.print("Pres: "); Serial.println(aux); return aux;}
float ds18b20value() {
...
void interrupt_sense() {
    ...
    **/
    alarm_flag |= SENSE_FLAG; //OR bit a bit y se guarda en alarm flag equivalent to x = x | y
}
...
void interrupt_send() {
    ...
    **/
    alarm_flag |= SEND_FLAG;  //OR bit a bit y se guarda en alarm flag equivalent to x = x | y
}
...
void interrupt_both() {
    ...
    **/
    alarm_flag |= (SENSE_FLAG|SEND_FLAG);  //OR bit a bit y se guarda en alarm flag equivalent to x = x | y
}
